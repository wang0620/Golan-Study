...
//The minimum precision for constant is 256 bits.
//They are considered to be mathematically exact.
...
func main() {
	...
	//Constant can be a typed or untyped for GO
	//When it is untyped, we consider it as a kind
	//They are implicitly converted by the compiler.

	//Untyped Constants
	const ui = 12345 // kind: integer
	const uf = 3.14159236 // kind: float

	fmt.Println(ui)
	fmt.Println(uf)

	//Typed Constant still use the constant type system
	//but their precision is restricted. 256b bit

	const ti int = 12345 //type int
	const tf float32 = 3.1415926// type float64
	...
	fmt.Println(myUint8)*/
	//Out: constant 1000 overflows uint8
	...
	var answer  = 3 * 3.03 // KindFloat(3)*KindFloat(3.03)
	fmt.Println(answer)
...
//----
	...
	const (
		Ldate         = 64 >> iota //  1 : Shift 1 to the left 0.  0000 0001
